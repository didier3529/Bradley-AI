...
function transformTokenPriceToMarketAsset(
  symbol: string,
  tokenPrice: any,
  balance: string = "1"
): PortfolioToken {
  ...
  return {
    ...
    priceChange24h:
      tokenPrice.historical !== 0
        ? ((tokenPrice.current - tokenPrice.historical) /
...
export async function GET(request: Request) {
  ...
  console.log(
    "🚀 MARKET_DATA_API: Forcing use of hardcoded prices from price-service.ts by removing cache logic."
  );
  ...
  try {
    ...
    const assetsForMarketData: PortfolioToken[] = hardcodedTokenSymbols
      ...
      .map((symbol) =>
        transformTokenPriceToMarketAsset(symbol, currentPrices[symbol])
      );

    if (assetsForMarketData.length === 0) {
      console.error(
        "🚀 MARKET_DATA_API: No hardcoded price data found after calling getTokenPrices."
      );
      throw new Error("No hardcoded price data found for market display.");
    }
    console.log(
      `🚀 MARKET_DATA_API: Successfully transformed ${assetsForMarketData.length} hardcoded assets.`
    );

    // Calculate some aggregate data based on our hardcoded assets
    const totalMarketCap = assetsForMarketData.reduce(
      (sum, asset) =>
        sum + asset.price * (parseFloat(asset.quantity) || 1000000),
      0
    );
    const btcData = assetsForMarketData.find((asset) => asset.symbol === "BTC");
    const btcDominance =
      btcData && totalMarketCap !== 0
        ? ((btcData.price * (parseFloat(btcData.quantity) || 1000000)) /
            totalMarketCap) *
          100
        : 0;
    const totalVolume24h =
      assetsForMarketData.reduce(
        (sum, asset) =>
          sum +
          (asset.priceChange24h !== 0
            ? Math.abs(
                asset.price *
                  (parseFloat(asset.quantity) || 1000000) *
    ...
    console.log(
      `🚀 MARKET_DATA_API: Calculated aggregates - TotalMCap: ${totalMarketCap}, BTCDom: ${btcDominance}, TotalVol: ${totalVolume24h}`
    );
    ...
    try {
      ...
      console.warn(
        "[DEBUG] Market API: Twitter API Error (fallback used):",
        error
      );
    ...
    const sortedByChange = [...assetsForMarketData].sort(
    ...
    const marketData: MarketData = {
      ...
      marketCapChange24h:
        assetsForMarketData.length > 0 &&
        assetsForMarketData[0].priceChange24h !== undefined
          ? assetsForMarketData[0].priceChange24h
          : 0,
      ...
      sentimentData: {
        ...
        socialMetrics: {
          ...
          reddit: {
            sentiment:
              btcData && btcData.priceChange24h !== undefined
                ? getSentiment(btcData.priceChange24h / 100)
                : "Neutral",
            volume24h: 12345,
            volumeChange24h:
              btcData && btcData.priceChange24h !== undefined
        ...
        technicalIndicators: {
          macd:
            btcData &&
            btcData.priceChange24h !== undefined &&
            btcData.priceChange24h > 0
              ? "buy"
              : "sell",
          rsi:
            btcData && btcData.priceChange24h !== undefined
              ? Math.min(100, Math.max(0, 50 + btcData.priceChange24h))
              : 50,
          movingAverages:
            btcData &&
            btcData.priceChange24h !== undefined &&
    ...
    console.log(
      `🚀 MARKET_DATA_API: Constructed marketData. BTC Price in topGainers/Losers (example): ${marketData.topGainers[0]?.price}`
    );
    ...
    console.error(
      "🚀 MARKET_DATA_API Error (forcing hardcoded prices path):",
      error
    );
    performanceMonitor.endOperation("market-data-api", operationId);
    return NextResponse.json(
      {
        error: "Failed to fetch market data using forced hardcoded prices",
