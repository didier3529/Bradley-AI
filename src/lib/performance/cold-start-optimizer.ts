...
export class ColdStartOptimizer {
  ...
  private initializeDefaultServices() {
    ...
    this.registerService({
      ...
      healthCheck: async () => {
        const response = await fetch('/api/health/portfolio', { method: 'HEAD' })
        return response.ok
      },
    })
    ...
    this.registerService({
      ...
      initializer: async () => {
        ...
        await Promise.all(symbols.map(symbol =>
          this.warmupService(`/api/prices/${symbol}`)
        ))
      },
    })
  ...
  private async progressiveLoadServices(): Promise<void> {
    const remainingServices = Array.from(this.services.values())
      ...
      .sort((a, b) => {
        ...
        return priorityDiff !== 0 ? priorityDiff : a.dependencies.length - b.dependencies.length
      })
    ...
    logger.info(`Progressive loading ${remainingServices.length} services`, {
      ...
      metadata: {
        services: remainingServices.map(s => ({ name: s.name, priority: s.priority })),
    ...
    for (let i = 0; i < remainingServices.length; i += batchSize) {
      ...
      if (i + batchSize < remainingServices.length) {
        await new Promise(resolve => setTimeout(resolve, ColdStartConfig.progressive.intervalMs))
      }
    }
  }

  private async loadService(serviceName: string, phase: 'critical' | 'progressive'): Promise<void> {
    ...
    try {
      ...
      await Promise.race([
        ...
        new Promise((_, reject) => {
          setTimeout(() => reject(new Error(`Service ${serviceName} load timeout`)), service.timeout)
        })
      ])
  ...
  private async waitForDependencies(dependencies: string[]): Promise<void> {
    ...
    const dependencyPromises = dependencies.map(async (depName) => {
      ...
      if (state.status === 'failed' && !this.services.get(depName)?.fallbackData) {
      ...
      for (const serviceName of warmupServices) {
        ...
        if (service) {
          try {
            ...
            logger.debug(`Background cache warming completed for: ${serviceName}`, {
              component: 'cold-start-optimizer',
  ...
  private getTimeToFirstPaint(): number {
    ...
    const criticalServiceStates = Array.from(this.loadingStates.values())
      .filter(state => {
  ...
  private getTimeToInteractive(): number {
    ...
    const highPriorityStates = Array.from(this.loadingStates.values())
      .filter(state => {
        const service = this.services.get(state.serviceName)
        return service?.priority === 'critical' || service?.priority === 'high'
      })

    const allHighPriorityLoaded = highPriorityStates.every(state =>
      state.status === 'loaded' || state.status === 'fallback'
    )
    ...
    return Math.max(...highPriorityStates.map(state => state.endTime || 0)) - this.startTime
  }
